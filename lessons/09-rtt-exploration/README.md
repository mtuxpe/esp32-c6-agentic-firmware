...
Deep dive into Real-Time Transfer (RTT) for embedded debugging and autonomous LLM development.

## Overview: Real-Time Transfer (RTT)

### What is RTT?

RTT (Real-Time Transfer) is a non-blocking debugging protocol that uses JTAG to communicate between host and device via in-memory ring buffers.

**Key characteristics:**
- Non-blocking - Device continues execution while logging
- Multi-channel - Separate channels for different data types
- Bidirectional - Host can send commands, device can respond
- No GPIO overhead - Uses internal JTAG connection
- High throughput - 1-10 MB/s vs UART's 14 KB/s

**Channels available:**
1. **Channel 0** - Structured logging (defmt)
2. **Channel 1** - Binary sensor data (unformatted, high-speed)
3. **Channel 2** - Memory probes (arbitrary device RAM reads)
4. **Channel 3+** - Device control (host → device commands)
...
- Production debugging (deployed systems)
